...
* Copy the contents of the deployments folder to `wildfly/standalone/deployments/`
	* The deployments folder contains a data source file (`wmsdemo-ds.xml`) and a json instance settings file (`wmsdemo.json`).
...
* You will need to create an addins directory either within your content directory (where Skyve will look by default), or configure `addins: { directory:` within your `wmsdemo.json` to the location of the addins directory.
...
* If you selected a database engine other than H2, you will need to create a schema (MySQL) or database name (MSSQL) matching your specified project name (wmsdemo). If you want to use a different database or schema name, you'll need to modify the `wmsdemo-ds.xml` file in the deployments folder accordingly.
